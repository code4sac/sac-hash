...
function backfillDates(doc) {
  if(doc[1]['$setOnInsert'].metadata.shortDate.length === 10) {
    return backfillHours(doc);
  } else if(doc[1]['$setOnInsert'].metadata.shortDate.length === 7) {
    return backfillDays(doc);
  } else {
    console.error('query:', doc[0]);
